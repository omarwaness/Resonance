...
#   --voice-key "voices/system/<actual-voice-id>"

# Use this to test with PowerShell:
# Invoke-WebRequest -Uri "https://<your-modal-endpoint>/generate" `
#   -Method Post `
#   -Headers @{"X-Api-Key"="<your-api-key>"} `
#   -ContentType "application/json" `
#   -Body '{"prompt":"Hello from Chatterbox [chuckle].","voice_key":"voices/system/<actual-voice-id>"}' `
#   -OutFile output.wav

# Use this to test with curl.exe on Windows:
# curl.exe -X POST "https://<your-modal-endpoint>/generate" \
#   -H "Content-Type: application/json" \
#   -H "X-Api-Key: <your-api-key>" \
#   -d "{\"prompt\":\"Hello from Chatterbox [chuckle].\",\"voice_key\":\"voices/system/<actual-voice-id>\"}" \
#   --output output.wav

# R2 cloud bucket mount (read-only, replaces Modal Volume)
R2_BUCKET_NAME = "audio"
R2_ACCOUNT_ID = "57488fa94e900a32d7b7c45631531961"
R2_MOUNT_PATH = "/r2"
r2_bucket = modal.CloudBucketMount(
    R2_BUCKET_NAME,
    bucket_endpoint_url=f"https://nvvtzgscknifdwnhnvez.storage.supabase.co/storage/v1/s3",
...
class Chatterbox:
    ...
    def serve(self):
        ...
        def generate_speech(request: TTSRequest):
            if "<" in request.voice_key or ">" in request.voice_key:
                raise HTTPException(
                    status_code=400,
                    detail="Replace <actual-voice-id> with a real voice id or stored r2ObjectKey.",
                )
